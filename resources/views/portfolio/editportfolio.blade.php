...
<span class="text-purple-800 font-bold text-xl">Edit Portfolio Data</span>
...
<div class="mb-6">
    ...
    <input id="title" name="title" type="text"  class="w-4/12 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
</div>
...
@section('script')
<script>
var portoId = window.location.href.split('/').pop();
document.addEventListener('DOMContentLoaded', function () {
fetch(`http://localhost:8000/api/portfolio/show`, {
    method: 'POST',
    headers: {
    ...
    .then(portoData => {
        // Populate form with porto data
        console.log(portoData);

        document.getElementById('title').value = portoData.data.title;
        document.getElementById('start').value = portoData.data.start;
        document.getElementById('end').value = portoData.data.end;
        document.getElementById('price').value = portoData.data.price;
         const radioButtons = document.getElementsByName('status');
        radioButtons.forEach(radio => {
            if (radio.value === portoData.data.status.toString()) {
                radio.checked = true;
            }
        });

    })
    .catch(error => console.error('Error fetching porto data:', error));
 ...
 formData.append('id',portoId);
    fetch('http://localhost:8000/api/portfolio/update', {
        method: 'POST',
        body: formData,
    })
    .then(response => response.json())
